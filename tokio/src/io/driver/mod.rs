...
cfg_rt! {
    impl Handle {
        ...
        pub(super) fn current() -> Self {
            let (_, handle) = crate::runtime::context::io_handle();
            handle.expect("A Tokio 1.x context was found, but IO is disabled. Call `enable_io` on the runtime builder to enable IO.")
        }
    }
}
